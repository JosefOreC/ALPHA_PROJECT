...
| combustible_total_l | DECIMAL(12,2) | |
| co2_total_kg | DECIMAL(12,2) | |
| generado_en | TIMESTAMP WITH TIME ZONE | DEFAULT CURRENT_TIMESTAMP |

### RUTA_REPORTE (tabla intermedia N:M)

| Atributo | Tipo | Restricción |
|---|---|---|
| ruta_id | UUID | FK → RUTA(ruta_id), parte de PK compuesta |
| reporte_id | UUID | FK → REPORTE(reporte_id), parte de PK compuesta |
...
CREATE TABLE zonas_restringidas (
    zona_restringida_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    nombre              VARCHAR(150) NOT NULL,
    descripcion         TEXT,
    tipo                VARCHAR(50) NOT NULL,
    geometria           GEOMETRY(POLYGON, 4326) NOT NULL,
...
CREATE INDEX idx_zonas_restringidas_geometria
ON zonas_restringidas
USING GIST (geometria);

CREATE INDEX idx_zonas_restringidas_estado
ON zonas_restringidas(estado);
...
CREATE TABLE ruta_reporte (
    ruta_id    UUID NOT NULL REFERENCES rutas(ruta_id) ON DELETE CASCADE,
    reporte_id UUID NOT NULL REFERENCES reportes(reporte_id) ON DELETE CASCADE,
    PRIMARY KEY (ruta_id, reporte_id)
);

CREATE INDEX idx_ruta_reporte_reporte
ON ruta_reporte(reporte_id);
---
```
...
|---|---|---|---|
| 1.0.0 | 04/09/2026 | Rojas Camayo, Valentino Jhan Pierre | Versión inicial: modelo conceptual, lógico (3FN) y físico DDL de entidades principales. |
| 2.0.0 | 18/09/2026 | Rojas Peña William Mikeiel | Versión 2: Implementacion de tablas faltantes. |
